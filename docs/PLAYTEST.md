...
This is an early **vertical slice** — the tactical core plus a thin run wrapper. It exists to answer one question: *is a fight fun, and is a short descent tense?* Most of the design bible (a real gear/inventory system, subclasses/feats, Charters/Covenants) is **not in this build yet** — see [ROADMAP.md](ROADMAP.md). For a deeper look at known gaps against the club-management vision and where to take the game next, see [QA_NOTES.md](QA_NOTES.md).
...
| System | State |
|---|---|
| **Main menu — 3 save slots** — the app opens on a club-select screen, not straight into whatever was last played. Each slot is a fully independent save (its own club, roster, league position, everything); Play loads one, Delete clears one (with a confirm). A pre-slots single save migrates into Slot 1 automatically, once. "Switch club" on the Settlement screen returns here | ✅ |
...
`?dev=enc&seed=<seed>` jumps straight into a deployed encounter (drafts a throwaway party via `Run`'s internal pool — that machinery still exists for dev/test use, it's just not reachable from the live UI any more). `?dev=boss&seed=<seed>` jumps straight into a Deep-5 boss fight. Add `&rounds=N` to auto-play N rounds first (auto-attacks the lowest-HP target in range and otherwise walks toward the nearest enemy). `?dev=standings` jumps straight to the league table. `?dev=market` jumps to the Recruitment Market with a seeded roster + treasury (also shows the Sump School section). `?dev=disband` jumps to the Disbandment screen. `?dev=recap` plays out a full season and jumps to the recap screen. `?dev=descentview&seed=<seed>` opens the Descent screen with a signed roster (add `&floor=N` to fight through to a given floor first, `&floors=N` to temporarily shrink the season dungeon so you can reach the final-floor state quickly). `?dev=debrief&seed=<seed>` fast-forwards a full run to the Debrief screen with a league fixture result including the wages line (add `&wipe=1` for the loss state, `&promo=1` to see a season-ending promotion, `&releg=1&wipe=1` for a relegation). `?dev=fullclear&seed=<seed>` plays a 1-floor season to its final-floor victory and stops on the Aftermath screen — the regression check for the "pending level-ups stranded on a full clear" bug (see below). Every dev shortcut always uses Slot 1, bypassing the main menu. For quick balance passes: `npm test` runs the headless multi-seed run simulation.

## Fixed this pass (softlocks + bugs)

An expert-tester pass through the club/league/season systems turned up four real issues, all fixed and covered by tests:

1. **Stranded level-ups on a full clear.** The season's final floor ends the run automatically (`state.over`), and the code skipped straight to the Debrief screen — bypassing the Aftermath screen, the *only* place level-ups get assigned. A club that kept fully clearing its dungeon would accumulate `pendingLevelUps` forever with no UI path to spend them, and those characters would silently stop growing (HP/AC/features never recompute until a level is actually assigned). Fixed: Aftermath now always shows after a win, and its "Onward" button routes to the Debrief instead of a (nonexistent) next floor when the run has already ended.
2. **Financial gridlock softlock.** Wages + a short roster could leave a club unable to afford even a single 60g Sump School signing, with no way to ever earn more (you can't play a fixture with an incomplete roster). Fixed: `checkDisbandment()` now also catches "roster short, can't afford to complete it," checked proactively every time the Settlement screen loads (not just right after a fixture) — a building purchase can create this state just as easily as a bad run.
3. **Rebuild-into-another-rebuild loop.** A club that disbanded while nearly penniless could get floored back into the exact same stuck state. Fixed: `disbandAndRebuild()` now guarantees enough gold to re-staff a full roster through the Sump School alone, every time — the same bootstrap guarantee a freshly founded club gets.
4. **False-positive disbandment on a fresh start.** Making the disbandment check proactive (fix #2) risked flagging a brand-new or just-rebuilt club's *empty-by-design* roster as a "wipeout." Fixed with a `fixturesThisIncarnation` counter — the wipeout check only fires for a club that has actually fielded a fixture before.
