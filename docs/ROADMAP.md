...
- [x] **The League** — reframes the run as a fixture in a competitive circuit (`game/league.ts`): a 4-division pyramid (Salvage Rounds → Prospect Wards → Lower Pit Circuit → Deep Vault League, fee rising with prestige) takes a fee off each fixture's gold before it hits the treasury; a standings table ranks the club against 5 rival clubs *per division* by clears → gold → net gold → squad health → reputation, with every division's rivals lightly simulated live. Every 6 fixtures a season settles: top 2 promote, bottom 2 relegate, season stats reset (career totals persist). Leveling stays purely combat-based — the league never grants XP, it only ranks.
- [x] **Persistent roster + Recruitment Market** (`game/recruitment.ts`) — survivors are signed to the club permanently instead of being redrafted every run; the free draft only fills open slots, while the Market lets you spend treasury gold to sign a specific priced-and-risk-tagged prospect, or release someone to free a slot.
- [x] **Team disbandment** — the club is forced to rebuild (roster wiped, most of the treasury gone, dropped to the bottom division, league record reset) if the roster is wiped out entirely or the club is broke after three straight fixture losses; the Ledger (graveyard, past runs, best depth) survives the collapse.
